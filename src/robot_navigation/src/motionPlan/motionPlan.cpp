...
void motionPlan::motionInit(void)
{
  ...
  {
    // 订阅真值里程计进行仿真
    localizationSub = motPlan.subscribe("/odom", 10, &motionPlan::odomCallback, this);
  }
  // 订阅局部地图
...
void motionPlan::odomCallback(const nav_msgs::OdometryConstPtr& msg)
{
    startPoint[0] = msg->pose.pose.position.x;
    startPoint[1] = msg->pose.pose.position.y;
    getStartFlag = true;
    // 可选：打印起点便于调试
    // ROS_INFO("odomCallback: start = (%.2f, %.2f)", startPoint[0], startPoint[1]);
}
